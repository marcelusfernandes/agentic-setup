...
| `human:decided` | The decision was recorded; work may proceed. Kept as the audit trail of the intervention. Named `decided`, not `reviewed`, so it is never mistaken for a code review (`review:approved`). |
...
`human:pending` and bare `human` are gates; `human:decided` never blocks and is never removed
...
replaces it with `human:decided` next to `state:done`; a new decision revision restores
`human:pending` and removes `human:decided`. On objectives, tasks and PRs the workflow never
...
transitive dependents — and only a person flips them to `human:decided`, after which the
...
dependents even if closed; one tagged `human:decided` does not. Either is read as a gate
...
originating label to `human:decided` with a comment linking the decision, and that label
...
`human:decided` when `labels` runs.
