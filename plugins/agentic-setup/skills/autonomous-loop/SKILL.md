...
4. **Review:** validate, then open a PR to the integration branch with `Closes #<task>`. Use an independent
   ...
   repository policy needs a human decision, record a scoped `human:pending` checkpoint
   for that setup; do not turn every routine PR into a new permission request.
5. **Land:** `land <objective> <task>` enforces the merge permission, unresolved
   `human:pending` requests/checkpoints, current-head approval and required server checks. Never bypass a
...
- Persist failed attempts and evidence in task comments. After two failed repair
  ...
  or resource. If a person must decide or supply it, record a linked `human:pending` checkpoint.
