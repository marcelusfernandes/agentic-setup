...
| PreToolUse Edit/Write | `protect-worktree.mts` | denies a subagent's write that resolves inside the main checkout but outside its own worktree. A real failure mode: under load the model writes with an absolute path rooted at the main repository, and a prose rule does not stop it. Outside the checkout it denies nothing — it is a fence around that one failure mode, not a sandbox — and since #327 it **names** the one outside path that is not scratch: a write under `~/.claude/agent-memory/<agent>/`, the `memory: user` scope, is allowed with a note saying so, and every main-checkout refusal carries that path, so an agent that knows only its own worktree learns from the refusal where a durable write goes. A denied path that is itself agent memory in the checkout (`.claude/agent-memory/`, `.claude/agent-memory-local/` — the `project` and `local` scopes) is told which scope would have survived. Every refusal that names the location says in the same breath that the scope is set in the agent's card and not at write time, because the directory is Claude Code's to fill and not one to hand-write into. What the hook does not promise is isolation: that directory is **one global location per card**, with no per-run, per-worktree, per-branch or per-repository component, and this hook cannot add one — the payload carries `agent_id`, never the card name that names the directory. The permit is withheld, and the path denied like any other, when the location resolves inside the checkout (a `$HOME` that contains it) or inside the worktree (a `$HOME` that resolves to nothing, which makes the path relative); the refusal then names no location at all, since a hole in this fence — or a "durable" location that dies with the pass — costs more than the naming |
...
- **An agent that declares `memory: project` writes its memory inside its worktree — and
  ...
  already existed. **What durability does not buy is isolation, and this is the sharper
  half of the trade.** `~/.claude/agent-memory/<agent>/` is one global directory per card:
  no per-run, per-worktree, per-branch or per-repository component, and the hook cannot
  supply one, because the payload it reads carries `agent_id` and never the card name that
  names the directory. Two implementers dispatched in parallel — the loop's normal mode,
  four issues with disjoint globs — are the same card, so they share one
  `implementer/MEMORY.md` and write over each other exactly the way the next bullet
  describes for a shared database: one agent's write lands under another's read. The same
  directory is also shared *across repositories*, so a card flipped to `user` carries what
  it learned here into every repository the plugin runs in. Both costs land on whoever
  flips the card, and neither card is flipped here: that choice is outside this bullet's
  mechanism question, and a memory that several agents write concurrently needs a
  convention for who owns which file before it is worth having.
