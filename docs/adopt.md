...
file is created, moved or touched, and every `gh` call it makes is a read. Three reads,
in this order:
...
- `hooks` — the hooks this setup installs (`pre-push`) that are present **and** carry its
  marker. Someone else's `pre-push` is not ours and is not listed. Worktrees are
  resolved through `commondir`, so the hooks reported are the ones git would run.
- `workflows` — every `*.yml`/`*.yaml` in `.github/workflows`, sorted; not only ours.
...
the second is true would remove a protection it never saw.
...
| *(gh's own first line)* | `gh issue create` itself failed |
