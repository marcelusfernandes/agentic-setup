...
- **Every decision after them is one dated file** in this directory, named
  ...
  the next free number is `0024-<slug>.md` — item 22 is claimed by #231 (PR #311) and its
  index row lands with it, so a gap at 22 here means that pull request has not merged
  yet. The date lives inside the file, on its
