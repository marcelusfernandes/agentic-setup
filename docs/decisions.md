...
that ran `land.mts` could write itself. **Item 17 supersedes this item's answer to that
...
**superseded by item 17: everything in this paragraph describes the opt-in `approved`
...
## 17. 2026-09-17: one review mode — an isolated agent, a label the orchestrator writes
...
under `decisions/`. This item lives here for the same reason the 2026-09-17 note under
item 13 does: #148's `## Files` lists `docs/decisions.md` and no path under
`decisions/`, and an implementer never widens its own globs. Its index row in
`decisions/README.md` is owed by whoever relocates items 14 to 17.
