...
// Date 2026-09-20. A scratch repository created for this measurement, owned by
// this repository's owner and left in place. It is cited and not named:
// invariant 7 refuses provenance, and a repository a reader cannot open does
// nothing else — the lesson below is complete without it. The four squash SHAs
// live in PR #438's body and in a comment on #435, neither of which is a
// tracked file. Settings on it:
...
//
// A — the escape in a **commit message body**, single-commit branch.
...
//     `COMMIT_OR_PR_TITLE` takes the PR title, not a commit subject), the title
//     POSTed as correctly JSON-encoded bytes through `gh api -X POST --input -`.
//     The subject line on the base carries 5c 75 30 30 30 30 — **intact**.
...
//     reads the stored body to build the commit — **intact**.
// D — the same through `gh api -X PATCH --input -` on an existing body, the
//     exact path #416 used — **intact**.
//
// Which setting was live at each merge is legible from the artefacts and not
// only from the read-back: B's subject is the pull request title on a
// two-commit branch, which only `COMMIT_OR_PR_TITLE` produces, and C's and D's
// bodies are the pull request body rather than the branch messages, which only
// `PR_BODY` produces. A and B ran under the production configuration; the
// temporary setting for C and D is restored and disclosed here.
//
...
// three (checked for D).
//
// **Proved on one object whose storage cannot be argued about.** Case A's
// squash commit is content-addressed, and git holds six bytes in it. Ask GitHub
// for that same commit twice:
//
//     Accept: application/vnd.github.patch  ->  5c 75 30 30 30 30  (six bytes)
//     the JSON representation               ->  5c 5e 40           (three)
//
// One unchanged object, two representations, only one of them lossy. That is
// the whole of the claim, on a value no model of the input path can explain
// away.
//
...
//
// **One step this does not prove, stated rather than glossed.** For a pull
// request *title* and *body* specifically, six-character storage is the
// simplest model consistent with everything visible, not a proof. B, C and D
// are the cases where GitHub reads its own stored title or body to build a
// commit and wrote the six bytes into it — but a double-decode on input plus a
// different re-encoding on each output is still coherent. Case A makes that
// unlikely, since a verbatim commit-building path cannot invent the escape for
// a stored U+0000; it does not exclude it. The discriminator is a pull request
// body carrying a genuine U+0000 byte, read back and squashed under `PR_BODY`.
// It was not run, because it would mean writing such a value into a repository
// to see what comes out, and the question it settles is not worth that.
//
...
// the same either way, but the queued path was not the one exercised. The
// residual worth naming: if GitHub snapshots the intended message when
// auto-merge is enabled, that snapshot is a round trip through the layer shown
// above to rewrite this text. It is not observable read-only, and no open pull
// request here carries an auto-merge request to inspect.
//
// Two placements were not run because they sit on the same git-object path case
// A already covers, and are judged low risk on that ground rather than on none:
// the escape in a commit **subject** on a single-commit branch, and in a commit
// **body** on a multi-commit branch, where the squash concatenates messages.
//
...
// restated here. **Two** sentences of that header are now superseded, both in
// its section headed "one layer up: GitHub, and whether a gate is owed", and a
// follow-up that fixes one fixes half the staleness:
//   - the sentence beginning "GitHub sanitises that escape in a pull request
//     body" asserts as measured the exact claim corrected above — and unlike
//     #427's and #435's bodies, that one sits in a tracked file;
//   - the paragraph beginning "What is still **unmeasured**" describes this
//     experiment as still to be run, and names the `ci/` gate that corruption
//     would have owed.
// `tests/tree-bytes.test.mts` is not in #435's `## Files`, so it is not edited
// here and no grant is widened; correcting both is the follow-up, and this file
// is where the answer lives until then.
